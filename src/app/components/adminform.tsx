...
export function FormShell({
  ...
  return (
    <>
      <Link
        href="/admin"
        className="-ml-2 inline-flex min-h-[40px] items-center gap-1 rounded-sm px-2 pt-6 text-small font-medium text-muted transition-colors duration-150 hover:text-ink"
      >
      ...
      <header className="pb-6 pt-4">
        ...
        <p className="mt-2 max-w-[56ch] text-small text-muted">{intro}</p>
      </header>
...
export function Fieldset({ legend, children }: { legend: string; children: React.ReactNode }) {
  return (
    <fieldset className="u-card overflow-hidden">
      <legend className="sr-only">{legend}</legend>
      <p className="border-b border-line bg-canvas-2 px-5 py-3 font-display text-small font-semibold">
        {legend}
      </p>
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start gap-4">
        <div className="relative h-32 w-32 shrink-0 overflow-hidden rounded border border-line bg-canvas-2">
          {value ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            disabled={uploading}
            className="inline-flex min-h-[44px] items-center gap-2 rounded-sm border border-line-strong px-4 font-display text-small font-semibold transition-colors duration-150 hover:bg-canvas-2 disabled:pointer-events-none disabled:opacity-45"
          >
          ...
          {value && (
            <button
              type="button"
              onClick={() => onChange("")}
              className="inline-flex min-h-[40px] items-center gap-2 px-1 text-caption font-medium text-sale-text transition-colors duration-150 hover:underline"
            >
